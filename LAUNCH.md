...
2. Add **all** env vars (Production). Use real values and **omit the test flags**:
   ...
   **Do NOT set** `LOCAL_TEST`, `SMS_DRY_RUN`, or `LLM_DRY_RUN` — leaving them off
   switches the app to the real Supabase / Twilio / Claude.
3. Deploy. `vercel.json` registers the every-5-min cron at `/api/cron/run-due`;
