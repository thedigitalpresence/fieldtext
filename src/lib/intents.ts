...
import { normalizeAmount, normalizePeriod, normalizeStatus, normalizeName, normalizeAddress } from "./normalize";
...
async function runAction(business: Business, p: ParsedAction, ctx: ParseContext, sourceMessageId: string | null): Promise<string> {
  switch (p.intent) {
    case "log_quote": return logQuote(business, p);
    case "update_status": return updateStatus(business, p);
...
async function logQuote(business: Business, p: ParsedAction): Promise<string> {
  const lang = businessLang(business);
...
async function updateStatus(business: Business, p: ParsedAction): Promise<string> {
  const lang = businessLang(business);
  if (!p.status) return lang === "es" ? "¿Qué estado le pongo — activo, completado o perdido?" : "What status — active, completed, or lost?";
  ...
  const c = matches[0];
  await updateClient(c.id, { status: p.status });
  if (p.status !== "quoted") await cancelQuoteReminders(c.id);
...
async function logPayment(business: Business, p: ParsedAction): Promise<string> {
  ...
  const client = matches.length === 1 ? matches[0] : null;
  const paidOn = p.paid_on ?? new Date().toISOString().slice(0, 10);
  await db().from("payments").insert({ business_id: business.id, client_id: client?.id ?? null, amount: p.amount, paid_on: paidOn });
  const who = client ? (lang === "es" ? ` de ${client.name}` : ` from ${client.name}`) : "";
  return t.paymentLogged(money(p.amount), who, paidOn, lang);
}
...
export async function buildSnapshot(business: Business): Promise<string> {
  ...
  const { data: paymentRows } = await db().from("payments").select("*").eq("business_id", business.id).order("created_at", { ascending: false }).limit(10);
  ...
  for (const c of active) lines.push(`- ${c.name}: ${money(c.amount)}${periodLabel(c.billing_period, lang)}${c.service_description ? `, ${c.service_description}` : ""}`);
