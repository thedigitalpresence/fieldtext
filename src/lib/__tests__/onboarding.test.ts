...
async function pendingSignup(phone: string, lang: string, name = "Miguel Torres", biz = "Torres Lawn Care") {
  await db().from("signups").insert({
    name, business_name: biz, phone, language: lang, status: "pending",
